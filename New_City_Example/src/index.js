...
function NewCityForm({ addCityName }) {
  const onSubmit = evt => {
    evt.preventDefault();
    const cityNameInput = evt.target.elements.cityName;
    addCityName(cityNameInput.value);
    cityNameInput.value = "";
  };
  return (
    <div>
...
class CityList extends React.Component {
  constructor() {
    ...
    this.state = {
      cityNames: ["Vancouver", "Toronto", "Victoria"]
    };
    this.addCityName = this.addCityName.bind(this);
  }

  addCityName(name) {
    let oldCityNames = this.state.cityNames;
    const newCityNames = [...oldCityNames, name];
    this.setState({ cityNames: newCityNames });
  }
